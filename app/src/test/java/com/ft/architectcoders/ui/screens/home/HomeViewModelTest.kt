...
class HomeViewModelTest {
    ...
    @Test
    fun `Movies are not requested if permission not granted`() =
        runTest {
            // Given
            val viewModel = HomeViewModel(fetchMoviesUseCase, regionRepository)
    ...
    @Test
    fun `Movies are requested when permission is granted`() =
        runTest {
            // Given
    ...
    @Test
    fun `Region is fetched from repository on success`() =
        runTest {
            // Given
            val movies = sampleMovies(1, 2, 3)
            whenever(fetchMoviesUseCase()).thenReturn(flowOf(movies))
    ...
    @Test
    fun `UseCase is invoked after permission granted`() =
        runTest {
            // Given
            val movies = sampleMovies(1, 2, 3)
            whenever(fetchMoviesUseCase()).thenReturn(flowOf(movies))
